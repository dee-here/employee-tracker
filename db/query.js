...
class Query {
  ...
  getAllDeptsAsSelectOptions() {
    return this.connection
      .promise()
      .query(`SELECT name AS name, id as value FROM department;`);
  }
  ...
  viewAllRoles() {
    return this.connection.promise()
      .query(`SELECT r.id, r.title, r.salary, d.name
        FROM role r
  ...
  viewCombinedSalaryForAllEmployeesPerDept() {
    return this.connection.promise()
      .query(`SELECT department.name, SUM(role.salary) AS total_budget
        FROM
        employee
  ...
  viewEmployeesByDept(department_id) {
    return this.connection.promise().query(`SELECT CONCAT(employee.first_name, " ", employee.last_name) AS name, department.name AS department, role.title, role.salary
        FROM
        employee
        JOIN role ON employee.role_id = role.id
        JOIN department ON  role.department_id = department.id
